...
void AsmHighlighter::highlightBlock(const QString& text)
{
    ...
    for (const auto& token : tokens) {
        if (regs.contains(token.trimmed().toString())) {
            setFormat(token.position(), token.length(), fmtReg);
        } else if (keywords.contains(token.trimmed().toString())) {
            setFormat(token.position(), token.length(), fmtKey);
        }
    }
    ...
    while (match.hasNext()) {
        auto m = match.next();
        setFormat(m.capturedStart(), m.capturedLength(), fmtNum);
    }
}
